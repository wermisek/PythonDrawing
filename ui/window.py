...
class Window:
    ...
    def _init_cpanel(self):
        ...
        for i in range(size):
            self._optvars[i].set(str(defaults[i]))
            self._optvars[i].trace('w', lambda *args, index=i: self._on_entry_change(index))
            self._optlabl[i].grid(column=0, row=(i * 2) + curr_row, columnspan=2, padx=5, pady=5, sticky='w')
            self._optentry[i].grid(column=0, row=(i * 2) + curr_row + 1, columnspan=2, padx=5, sticky='ew')
        curr_row += size * 2
    ...
    def _on_entry_change(self, index):
        try:
            val = float(self._optvars[index].get())
            # Clamp the value between min and max
            val = max(self._options[index][2], min(self._options[index][3], val))
            self.bot.settings[index] = val = round(val, 3)
            self._optlabl[index]['text'] = f"{self._options[index][0]}: {val}"
            self.tlabel['text'] = Window._SLIDER_TOOLTIPS[index]
        except ValueError:
            # If input is not a valid number, reset to default
            self._optvars[index].set(str(self._options[index][1]))
    ...
    def start(self):
        try:
            t = time.time() 
            cmap, estimated_time = self.bot.process(self._imname, flags=self.draw_options, mode=self._mode)
            
            # Format estimated time into hours, minutes and seconds
            hours = int(estimated_time // 3600)
            minutes = int((estimated_time % 3600) // 60)
            seconds = int(estimated_time % 60)
            
            if hours > 0:
                time_str = f"{hours} hour{'s' if hours != 1 else ''}, {minutes} minute{'s' if minutes != 1 else ''} and {seconds} second{'s' if seconds != 1 else ''}"
            elif minutes > 0:
                time_str = f"{minutes} minute{'s' if minutes != 1 else ''} and {seconds} second{'s' if seconds != 1 else ''}"
            else:
                time_str = f"{seconds} second{'s' if seconds != 1 else ''}"
            
            if not messagebox.askokcancel(self.title, 
                f'Estimated drawing time: {time_str}\n\nPress OK to start drawing (ESC to stop the bot).'
            ):
                self._set_busy(False)
                return
